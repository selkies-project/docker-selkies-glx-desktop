...
The [Selkies base container](https://github.com/selkies-project/selkies/blob/main/docs/components/base-image.md) supplies everything but the desktop and the X server: PipeWire audio, the GPU runtime wiring for NVIDIA, VA-API and Vulkan, the gamepad and webcam plumbing, [s6](https://skarnet.org/software/s6/) service supervision, an embedded [coTURN](https://github.com/coturn/coturn) server for the WebRTC transport, and Selkies itself. This image replaces the base's framebuffer server with an X.Org server on the GPU (the `xorg` service, configured at start by `selkies-xorg-config` for the GPU the session was given), and adds KDE Plasma on X11 (`plasma-desktop` with Dolphin, Konsole, KWrite, Gwenview, Ark and System Settings), Firefox and Google Chrome, and the [proot-apps](https://github.com/linuxserver/proot-apps) runner behind the dashboard's apps panel, which installs portable applications into the home directory without touching the image, Steam behind a bubblewrap stand-in that needs no user namespaces, Wine Staging with winetricks, VLC, LibreOffice and the KDE application set.

The NVIDIA X server modules (`nvidia_drv.so` and the GLX server module) come in with the driver's libraries from the NVIDIA Container Toolkit v1.20.1 or higher; under a runtime that injects the libraries alone, the first start on an NVIDIA GPU lifts the two out of the driver installer matching the host driver version, the only other place they are shipped, and a container that keeps its filesystem keeps them across restarts. The same installer supplies the driver's 32-bit client libraries where the runtime brings none in, so the first start may fetch it either way. `NVIDIA_DRIVER_VERSION` overrides the version to fetch when the host's cannot be read.
